...
try:
    ...
    # 注意：请确保这个路径是正确的
    base_path = '/home/luoew/model_output/14s' 
    df = pd.read_csv(f'{base_path}/{model_version}/analyze/reorganized_predictions.csv')
    ...
    # 为新的曲线图创建一个专用目录
    output_dir = f'{base_path}/{model_version}/analyze/visualization/wind_speed_plots_10min_flattened_smoothed' # 建议用新目录
    ...
    for group_name, group_df in df.groupby(grouper):
        ...
        # --- 新增：应用 Savitzky-Golay 滤波器 ---
        n_points = len(pred_series.values)
        
        # 仅当数据点数 > 窗口长度时才应用滤波，否则S-G会报错
        if n_points > FILTER_WINDOW_LENGTH:
            try:
                # 对预测值（pred_series）的值应用滤波器
                smoothed_pred_values = savitzky_golay(pred_series.values, 
                                                      window_length=FILTER_WINDOW_LENGTH, 
                                                      polyorder=FILTER_POLYORDER)
                
                # 更新图例标签
                pred_label = 'Predicted Wind Speed (Smoothed)'
                
            except Exception as filter_e:
                # 如果滤波失败（例如窗口设置不当），则退回使用原始数据
                print(f"警告: 在 {group_name} 应用S-G滤波失败: {filter_e}。将使用原始数据。")
                smoothed_pred_values = pred_series.values
                pred_label = 'Predicted Wind Speed (Raw)'
        else:
            # 数据点太少，不进行滤波
            if n_points > 0: # 仅在有数据时打印信息
                print(f"信息: 在 {group_name} 跳过滤波，数据点 ({n_points}) 不足 {FILTER_WINDOW_LENGTH}。")
            smoothed_pred_values = pred_series.values
            pred_label = 'Predicted Wind Speed (Raw)'
        # --- 滤波结束 ---
        ...
        # --- 修改：使用平滑后的数据和新标签进行绘制 ---
        ax.plot(pred_series.index, smoothed_pred_values, label=pred_label, color='royalblue', linewidth=1.8, alpha=0.9)
        # ---
    ...
    print(f"错误：无法找到 '{base_path}/{model_version}/analyze/reorganized_predictions.csv' 文件。请检查路径。")
except ImportError:
    print("错误：缺少 'scipy' 库。请运行 'pip install scipy' 来安装它。")

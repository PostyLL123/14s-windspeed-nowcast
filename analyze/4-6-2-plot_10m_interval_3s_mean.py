...
parser = argparse.ArgumentParser(
    description='plot consecutive time seiries at interval of 5s truncate',
    formatter_class=argparse.ArgumentDefaultsHelpFormatter # 使帮助信息更友好
)
parser.add_argument('--model_version', '-mv', type=str, help='Version name of the first model.', default = 'enhanced_s2s')
...
parser.add_argument('--base_dir', '-bd', type=str, default='/home/luoew/model_output/',
                    help='Base directory where model outputs are stored.')
parser.add_argument('--output_file', '-o', type=str, default='3s mean series.png', help='Name of the output plot file.')
parser.add_argument('--project', '-p', default='14s/', help='project dir')
args = parser.parse_args()
args.output_dir = os.path.join(args.base_dir, args.project, args.model_version, 'analyze/visualization/time series/')
os.makedirs(args.output_dir, exist_ok=True)
# --- 1. 数据加载与预处理 ---
try:
    # 加载数据集
    #model_version = 'S2S-decom-251011_1104-sampling-revised-residual'
    file_path = os.path.join(args.base_dir, args.project, args.model_version,'analyze/reorganized_predictions.csv')
    ...
    for group_name, group_df in df.groupby(grouper):
        ...
        for index, row in group_df.iterrows():
            ...
            true_values_for_row = row['true_t1':'true_t14'].rolling(window=5, center=True, min_periods=1).mean().values[:5]
            
            # 提取未来1到5秒的预测值，并计算其对应的时间戳
            pred_timestamps_for_row = [index + pd.Timedelta(seconds=i) for i in range(1, 6)]
            pred_values_for_row = row['pred_t1':'pred_t14'].rolling(window=5, center=True, min_periods=1).mean().values[:5]
        ...
        #true_series_3s = true_series.rolling(window='3s', min_periods=1, center=True).mean()
        ...
        # 保存图表
        filename = f"wind_speed_{args.output_file}_{start_time_str_file}_to_{end_time_str_file}.png"
        print(args.output_dir+filename)
        plt.savefig(args.output_dir+filename, dpi=200)
        plt.close(fig)
        generated_files.append(filename)

    print(f"成功生成 {len(generated_files)} 张图表，已保存至 '{args.output_dir}' 文件夹。")
